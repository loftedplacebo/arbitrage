...
from binance_extreme_funding.config import DEFAULT_CONFIG, BinanceExtremeFundingConfig
from binance_extreme_funding.models import parse_float, utc_now
...
<head>
  ...
  <title>Binance Extreme Funding</title>
  <style>
    :root { --bg:#f4f5f2; --panel:#fff; --ink:#171914; --muted:#697066; --line:#dfe3dc; --accent:#16734b; --warn:#a55f00; --bad:#b4362d; }
    * { box-sizing:border-box; }
    body { margin:0; background:var(--bg); color:var(--ink); font:14px/1.45 Inter,Segoe UI,Arial,sans-serif; letter-spacing:0; }
    header { background:#181b17; color:#fff; padding:20px 28px 14px; border-bottom:4px solid #f3ba2f; }
    header h1 { margin:0; font-size:24px; font-weight:650; }
    header p { margin:4px 0 0; color:#c8cec5; }
    nav { display:flex; gap:4px; padding:12px 28px 0; background:#181b17; overflow:auto; }
    nav button { border:0; border-bottom:3px solid transparent; background:transparent; color:#bdc4b9; padding:10px 14px; cursor:pointer; font-weight:600; }
    nav button.active { color:#fff; border-color:#f3ba2f; }
    main { padding:18px 28px 36px; max-width:1600px; margin:auto; }
    .toolbar { display:flex; align-items:center; justify-content:space-between; gap:12px; margin-bottom:14px; }
    .status { color:var(--muted); }
    button.refresh { border:1px solid var(--line); background:var(--panel); border-radius:4px; padding:8px 12px; cursor:pointer; }
    ...
    th,td { text-align:left; padding:10px 12px; border-bottom:1px solid #eceee9; white-space:nowrap; }
    th { position:sticky; top:0; background:#f8f9f7; color:#565d53; font-size:12px; text-transform:uppercase; }
    tr:last-child td { border-bottom:0; }
    .good { color:var(--accent); font-weight:650; } .warn { color:var(--warn); font-weight:650; } .bad { color:var(--bad); font-weight:650; }
    .pill { display:inline-block; border:1px solid var(--line); border-radius:999px; padding:2px 7px; font-size:12px; }
    .empty { padding:30px; text-align:center; color:var(--muted); }
    .hidden { display:none; }
    @media(max-width:800px) { header,nav,main { padding-left:14px; padding-right:14px; } .metrics { grid-template-columns:repeat(2,minmax(0,1fr)); } .toolbar { align-items:flex-start; } }
  </style>
</head>
<body>
  <header><h1>Binance Extreme Funding</h1><p>Independent live funding, basis and paper execution</p></header>
  <nav>
  ...
  <main>
    <div class="toolbar"><div id="status" class="status">Loading...</div><button id="refresh" class="refresh">Refresh</button></div>
    <div id="metrics" class="metrics"></div>
    <div class="table-wrap"><table><thead id="head"></thead><tbody id="body"></tbody></table></div>
  </main>
  <script>
    const $=s=>document.querySelector(s), esc=v=>String(v??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));
    const pct=v=>v===null||v===""||v===undefined?"-":Number(v).toFixed(4)+"%";
    const usd=v=>v===null||v===""||v===undefined?"-":Number(v).toLocaleString(undefined,{style:"currency",currency:"USD"});
    ...
    const columns={
      ...
      "daily-pnl":[["date_utc","UTC date"],["basis_realised_pnl_usd","Basis PnL"],["funding_realised_pnl_usd","Funding PnL"],["total_realised_pnl_usd","Total realised"],["exit_count","Exit events"],["exit_notional_usd","Exited notional"],["legacy_unattributed_pnl_usd","Legacy unattributed"]],
    ...
    function val(v,k){ if(k.includes("pnl_usd")||k==="exit_notional_usd") return usd(v); if(k.includes("rate")||k.includes("basis")||k.includes("pnl_pct")) return pct(v); if(k==="date_utc") return v??"-"; if(k.includes("_utc")) return dt(v); if(k==="minutes_to_funding") return v==null?"-":Number(v).toFixed(1); return v??"-"; }
    function render(payload){
      const rows=payload.items||[]; const cols=columns[active];
      $("#head").innerHTML="<tr>"+cols.map(c=>`<th>${esc(c[1])}</th>`).join("")+"</tr>";
      $("#body").innerHTML=rows.length?rows.map(r=>"<tr>"+cols.map(c=>`<td class="${cls(r[c[0]],c[0])}">${esc(val(r[c[0]],c[0]))}</td>`).join("")+"</tr>").join(""):`<tr><td class="empty" colspan="${cols.length}">No records yet</td></tr>`;
      $("#metrics").innerHTML=(payload.metrics||[]).map(m=>`<div class="metric"><span>${esc(m.label)}</span><strong>${esc(m.value)}</strong></div>`).join("");
      $("#status").textContent=`Updated ${dt(payload.observedAtUtc)}`;
    }
    async function load(){ try { const r=await fetch(`/api/${active}`,{cache:"no-store"}); const p=await r.json(); if(!r.ok) throw Error(p.error||r.statusText); render(p); } catch(e){ $("#status").textContent=e.message; } }
    document.querySelectorAll(".tab").forEach(b=>b.onclick=()=>{document.querySelectorAll(".tab").forEach(x=>x.classList.remove("active"));b.classList.add("active");active=b.dataset.tab;load();});
    $("#refresh").onclick=load; setInterval(load,30000); load();
  </script>
</body></html>"""
...
def _funding_payload(config: BinanceExtremeFundingConfig) -> dict:
    ...
    return {"observedAtUtc": utc_now().isoformat(), "items": items, "metrics": [
        ...
        {"label": "Largest absolute", "value": f"{max((abs(parse_float(row.get('current_funding_rate_pct'), 0.0) or 0.0) for row in items), default=0):.4f}%"},
    ]}


def _shortlist_payload(config: BinanceExtremeFundingConfig) -> dict:
    store = PaperStore(config)
    items = list(store.load_signals().values())
...
def _positions_payload(config: BinanceExtremeFundingConfig) -> dict:
...
def _daily_pnl_payload(config: BinanceExtremeFundingConfig) -> dict:
    ...
    def day_for(row: dict) -> dict:
        ...
        return days.setdefault(day, {
            "date_utc": day,
            "basis_realised_pnl_usd": 0.0,
            "funding_realised_pnl_usd": 0.0,
            "total_realised_pnl_usd": 0.0,
            "legacy_unattributed_pnl_usd": 0.0,
            "exit_count": 0,
...
def _summary_payload(config: BinanceExtremeFundingConfig) -> dict:
    ...
    items = [
        {"label": "Settled extreme events", "value": len(events)},
        {"label": "Compared observations", "value": len(comparisons)},
        {"label": "Displayed direction held", "value": f"{(same / len(comparisons) * 100 if comparisons else 0):.1f}%"},
        {"label": "Mean displayed error", "value": f"{mean_error:.4f}%"},
        {"label": "Basis take profit", "value": f"{config.basis_take_profit_pct:.2f}%"},
    ...
    return {"observedAtUtc": utc_now().isoformat(), "items": items, "metrics": [
        ...
        {"label": "Direction held", "value": f"{(same / len(comparisons) * 100 if comparisons else 0):.1f}%"},
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the Binance extreme-funding dashboard.")
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8770)
    args = parser.parse_args()
    server = ThreadingHTTPServer((args.host, args.port), DashboardHandler)
    print(f"Binance extreme-funding dashboard at http://{args.host}:{args.port}/", flush=True)
