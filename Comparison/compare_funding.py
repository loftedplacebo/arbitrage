...
def calculate_executable_entry_basis_pct(long_avg_price, short_avg_price):
    """
    Positive means favourable:
    we buy/long lower than we sell/short.
    """
...
def simulate_funding_direction_for_notional(
    long_orderbook,
    short_orderbook,
    ...
    """
    Simulates the funding arb opening direction:
    - buy/long on preferred long exchange using asks
    - sell/short on preferred short exchange using bids

    Net = funding gap + executable entry basis - estimated round-trip fees.
    """
    ...
    entry_basis_pct = calculate_executable_entry_basis_pct(
    ...
    net_after_fees_pct = (
        ...
        if entry_basis_pct is not None
        else None
    )
...
def classify_notional_simulation(notional_simulation):
    """
    Classification based on which notional levels are still positive.
    """
    def is_positive(notional):
        result = notional_simulation.get(notional, {})
        net = result.get("net_after_fees_pct")

        return (
            result.get("fully_filled") is True
            and net is not None
            and net > 0
        )
...
def add_live_execution_check(row):
    ...
    for notional in TEST_NOTIONALS_USDT:
        notional_simulation[notional] = simulate_funding_direction_for_notional(
            long_orderbook=long_book,
            short_orderbook=short_book,
            target_notional_usdt=notional,
            funding_gap_pct=row["abs_gap_pct"],
        )

    notional_classification = classify_notional_simulation(notional_simulation)
    ...
    row.update({
        ...
        "notional_classification": notional_classification,
    })
...
def get_sim_net(row, notional):
    simulation = row.get("notional_simulation", {})
    result = simulation.get(notional, {})
    return result.get("net_after_fees_pct")
...
def insert_execution_check_snapshot(timestamp, row):
    ...
    """, (
        ...
        row.get("notional_classification"),
        get_sim_net(row, 100),
        get_sim_net(row, 500),
        get_sim_net(row, 1000),
        get_sim_net(row, 2500),
        get_sim_net(row, 5000),
    ))
...
def format_sim_value(row, notional):
    simulation = row.get("notional_simulation", {})
    result = simulation.get(notional, {})
...
def print_notional_simulation_table(rows):
    print("\nFunding notional execution simulation")
    print(
        "Symbol       "
        "Long       "
        "Short      "
    ...
    for row in rows:
        print(
            f"{row['symbol']:<12}"
            f"{row['long_exchange']:<11}"
            f"{row['short_exchange']:<11}"
            f"{row.get('notional_classification', ''):<11}"
            f"{format_sim_value(row, 100):>9}   "
            f"{format_sim_value(row, 500):>9}   "
            f"{format_sim_value(row, 1000):>9}   "
            f"{format_sim_value(row, 2500):>9}   "
            f"{format_sim_value(row, 5000):>9}"
        )
